...
Class Master extends DBConnection {
	...
	function delete_sched(){
		...
		$qry = $this->conn->query("SELECT * FROM schedule_list WHERE id = '{$id}'");
		if($qry->num_rows > 0){
			$schedule_data = $qry->fetch_assoc();
			// Insert into the canceled_schedule column
			$data = json_encode($schedule_data, JSON_UNESCAPED_UNICODE);
			$insert_canceled = $this->conn->query("INSERT INTO canceled_schedule (schedule_data, canceled_at) VALUES ('{$data}', NOW())");
			if($insert_canceled){
				// Proceed to delete the original schedule
				$delete = $this->conn->query("DELETE FROM schedule_list WHERE id = '{$id}'");
				if($delete){
					$resp['status'] = 'success';
					$this->settings->set_flashdata('success', "Schedule successfully deleted and recorded.");
				} else {
					$resp['status'] = 'failed';
					$resp['error'] = $this->conn->error;
				}
